...
void SFMLSoundProvider::ToggleSounds() {
	if (_soundFileCache.GetSong(_currentSongName)->getStatus() == sf::Music::Playing) {
		
		if (_soundFileCache.GetSong(_currentSongName)->getVolume() > 0) {
			_soundFileCache.GetSong(_currentSongName)->setVolume(0);
		}
		else {
			_soundFileCache.GetSong(_currentSongName)->setVolume(100);
		}
	}
	else  {
		//_soundFileCache.GetSong(_currentSongName)->stop();
	}
}
